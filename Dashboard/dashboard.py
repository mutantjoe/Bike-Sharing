...
def initialize_daily_share_df(df):
    daily_share_df = df.groupby(by='dateday').agg({
        'counts': 'sum'
    }).reset_index()
    return daily_share_df

#Initialize the daily casual rent function
def initialize_casual_share_df(df):
    casual_share_df = df.groupby(by='dateday').agg({
        'casual': 'sum'
    }).reset_index()
    return casual_share_df

#Initialize the daily registered rent function
def initialize_registered_share_df(df):
    registered_share_df = df.groupby(by='dateday').agg({
        'registered': 'sum'
    }).reset_index()
    return registered_share_df

#Initialize the season rent function
def initialize_season_share_df(df):
    season_share_df = df.groupby(by='season')[['registered', 'casual']].sum().reset_index()
    return season_share_df

#Initialize the monthly rent function
def initialize_monthly_share_df(df):
    monthly_share_df = df.groupby(by='months').agg({
        'counts': 'sum'
    })
    ordered_months = [
    ...
    monthly_share_df = monthly_share_df.reindex(ordered_months, fill_value=0)
    return monthly_share_df

#Initialize the weekday rent function
def initialize_weekday_share_df(df):
    weekday_share_df = df.groupby(by='weekday').agg({
        'counts': 'sum'
    }).reset_index()
    return weekday_share_df

# Initialize the working day function
def initialize_workingday_share_df(df):
    workingday_share_df = df.groupby(by='workingday').agg({
        'counts': 'sum'
    }).reset_index()
    return workingday_share_df

# Initialize the holiday rent function
def initialize_holiday_share_df(df):
    holiday_share_df = df.groupby(by='holiday').agg({
        'counts': 'sum'
    }).reset_index()
    return holiday_share_df

# Initialize the weather rent function
def initialize_weather_share_df(df):
    weather_share_df = df.groupby(by='weather_situation').agg({
        'counts': 'sum'
    })
    return weather_share_df
...
daily_share_df = initialize_daily_share_df(main_df)
casual_share_df = initialize_casual_share_df(main_df)
registered_share_df = initialize_registered_share_df(main_df)
season_share_df = initialize_season_share_df(main_df)
monthly_share_df = initialize_monthly_share_df(main_df)
weekday_share_df = initialize_weekday_share_df(main_df)
workingday_share_df = initialize_workingday_share_df(main_df)
holiday_share_df = initialize_holiday_share_df(main_df)
weather_share_df = initialize_weather_share_df(main_df)
